...
export default function SSEExample() {
  ...
  const { data: allData, error: allError } = useSSE('/api/sse');
  const { data: evenData, error: evenError } = useSSE('/api/sse', 'even');
  const { data: oddData, error: oddError } = useSSE('/api/sse', 'odd');
